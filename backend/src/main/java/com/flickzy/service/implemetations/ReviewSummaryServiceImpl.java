...
public class ReviewSummaryServiceImpl implements ReviewSummaryService {
    ...
    public ReviewSummaryResponseDTO summarizeReviews(ReviewSummaryRequestDTO request) {
        ...
        // Lấy danh sách reviews từ DB
        List<Reviews> reviews = reviewRepository.findByMovie_Id(request.getMovieId(), Pageable.unpaged()).getContent();

        if (reviews.isEmpty()) {
            logger.error("No reviews found for movieId: {}", request.getMovieId());
            throw new IllegalArgumentException("No reviews found for this movie");
        }

        // Tính average rating
        double averageRating = reviews.stream()
                .mapToInt(Reviews::getStar)
                .average()
                .orElse(0.0);
        averageRating = Math.round(averageRating * 10.0) / 10.0;

        // Build reviews text cho prompt
